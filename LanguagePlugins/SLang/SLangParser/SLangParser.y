...
%output = SLangParserYacc.cs
%partial
%parsertype SLangGPPGParser
...
%token <ti> ZADACHA  // "Задача"
%token <ti> VVOD     // "Ввод"
%token <ti> PROVERKA // "Проверка"
%token <ti> TESTY    // "Тесты"
%token <ti> VYVOD    // "Вывод"

%token <ti> INDENT UNINDENT
...
%token <op> AND OR NOT
%token <ti> TASK INPUT SOLUTION TESTS OUTPUT
...
%type <op> assign_type
%type <stn> task_list task_decl task_body input_section solution_section tests_section output_section
	...
	| task_list
            {
                ...
                root = NewProgramModule(null, null, null, new block(null, stl, @$), null, @$);
            }
            ;
...
task_decl
    : TASK STRINGNUM LBRACE task_body RBRACE
    {
        var taskName = ($2 as literal)?.ToString(); 
	var bodyStmts = $4 as statement_list; 

	var stl = new statement_list();
	
	// Создаём blockStmt, чтобы упаковать bodyStmts в block
	var blockStmt = new block(null, bodyStmts, @$);
    ...
    ;
//----------- ТЕЛО ЗАДАЧИ DSL (4 основные секции) -----------
task_body
    : input_section solution_section tests_section output_section
    {
        var stl = new statement_list();
        if ($1 != null) stl.subnodes.Add($1 as statement);
        if ($2 != null) stl.subnodes.Add($2 as statement);
        if ($3 != null) stl.subnodes.Add($3 as statement);
        if ($4 != null) stl.subnodes.Add($4 as statement);
        $$ = stl;
    }
    ;
// Каждая секция (input, solution, tests, output) — упрощённо парсит { stmt_list }
input_section
    : INPUT LBRACE stmt_list RBRACE
    {
        $$ = $3;
    }
    | /* нет секции input */ 
...
solution_section
    : SOLUTION LBRACE stmt_list RBRACE
    {
        $$ = $3;
    }
    | /* нет */ 
...
output_section
    : OUTPUT LBRACE stmt_list RBRACE
    {
        $$ = $3;
    }
    | /* нет */ 
...
import_clause_one	: USE ident SEMICOLON {
